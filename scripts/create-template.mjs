/**
 * create-template.mjs — 从当前 initialData.ts 反向生成 resume-data.xlsx 模板
 * 只需运行一次，之后用户只需编辑 xlsx 即可
 */
...
const data = {
  ...
  experience: [
    {
      id: "exp_1", company: "领航创想智能科技有限公司", role: "资深 HRBP 专家 / 薪酬绩效部负责人",
      startDate: "2022-04", endDate: "至今", location: "深圳",
    ...
    {
      id: "exp_2", company: "极光数字空间科技有限公司", role: "HRBP 经理 (产研事业部负责人)",
      startDate: "2018-09", endDate: "2022-03", location: "深圳",
    ...
    {
      id: "exp_3", company: "中科智能制造控股集团", role: "人力资源主管 -> 薪酬福利高级专员",
      startDate: "2015-07", endDate: "2018-08", location: "上海",
  ...
  education: [
    {
      id: "edu_1", institution: "中南财经政法大学", degree: "本科", major: "人力资源管理",
      startDate: "2011-09", endDate: "2015-06", category: "学历教育",
  ...
  githubConfig: {
    username: "xiaomin-hrbp", useLiveApi: false,
    repoCountOverride: 12, commitCountOverride: 184, starredCountOverride: 45,
    languages: "薪酬福利设计与规划 (C&B):38.5:#10b981;全面绩效激励机制对齐 (KPI/OKR):28.2:#3b82f6;HRBP 业务诊断与组织发展 (OD):18.3:#f59e0b;人效效能分析系统 (BI & Analytics):10.0:#8b5cf6;劳动合同合规与用工风险预防:5.0:#ef4444",
  },
};
...
// 1. personalInfo
const piData = Object.entries(data.personalInfo).map(([key, value]) => ({ key, value }));
const piWs = XLSX.utils.json_to_sheet(piData);
piWs["!cols"] = [{ wch: 15 }, { wch: 80 }];
XLSX.utils.book_append_sheet(wb, piWs, "personalInfo");

// 2. skills
const skWs = XLSX.utils.json_to_sheet(data.skills);
skWs["!cols"] = [{ wch: 40 }, { wch: 8 }, { wch: 20 }, { wch: 12 }];
XLSX.utils.book_append_sheet(wb, skWs, "skills");

// 3. experience
const expWs = XLSX.utils.json_to_sheet(data.experience);
expWs["!cols"] = [{ wch: 8 }, { wch: 25 }, { wch: 30 }, { wch: 12 }, { wch: 10 }, { wch: 8 }, { wch: 80 }, { wch: 40 }];
XLSX.utils.book_append_sheet(wb, expWs, "experience");

// 4. projects
const projWs = XLSX.utils.json_to_sheet(data.projects);
projWs["!cols"] = [{ wch: 8 }, { wch: 35 }, { wch: 50 }, { wch: 18 }, { wch: 30 }, { wch: 8 }, { wch: 30 }, { wch: 15 }, { wch: 50 }, { wch: 50 }, { wch: 50 }, { wch: 50 }, { wch: 50 }];
XLSX.utils.book_append_sheet(wb, projWs, "projects");

// 5. education
const eduWs = XLSX.utils.json_to_sheet(data.education);
eduWs["!cols"] = [{ wch: 8 }, { wch: 20 }, { wch: 10 }, { wch: 15 }, { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 15 }, { wch: 15 }, { wch: 12 }, { wch: 50 }];
XLSX.utils.book_append_sheet(wb, eduWs, "education");

// 6. githubConfig
const ghData = [
  { key: "username", value: data.githubConfig.username },
  { key: "useLiveApi", value: String(data.githubConfig.useLiveApi) },
  { key: "repoCountOverride", value: String(data.githubConfig.repoCountOverride) },
  { key: "commitCountOverride", value: String(data.githubConfig.commitCountOverride) },
  { key: "starredCountOverride", value: String(data.githubConfig.starredCountOverride) },
  { key: "languages", value: data.githubConfig.languages },
];
const ghWs = XLSX.utils.json_to_sheet(ghData);
ghWs["!cols"] = [{ wch: 25 }, { wch: 80 }];
XLSX.utils.book_append_sheet(wb, ghWs, "githubConfig");

// ─── 写入文件 ───
XLSX.writeFile(wb, OUT_PATH);
console.log(`✅ Excel 模板已生成: ${OUT_PATH}`);
console.log("   包含 6 个 Sheet: personalInfo, skills, experience, projects, education, githubConfig");
